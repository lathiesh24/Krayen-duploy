    ...
    <div className="flex flex-col items-center justify-center min-h-screen max-w-screen-2xl">
        <h2 className='mb-[70px]'>All Posts</h2>
          {posts.map((post) => {
            ...
            return (
              <li key={post.id} className='flex flex-col max-w-[750px] rounded-lg bg-white shadow-lg mb-20 transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-105 hover:drop-shadow-2xl duration-300 cursor-pointer p-5'>
                 ...
                 <div className="flex max-h-[210px]">
                  ...
                  {Object.values(post.properties).map((property)=>{
                    if(property?.type == 'multi_select'){
                      const multidata = property?.multi_select?.map((values)=>{
                        console.log('values',values)
                          return (
                            <div className='cursor-pointer m-2 pl-2 pr-2 pb-1 min-w-[50px]  bg-[#89cff0] rounded-sm'>{values?.name}</div>
                          )
                      })
